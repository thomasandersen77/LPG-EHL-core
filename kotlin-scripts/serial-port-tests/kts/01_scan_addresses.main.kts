...
val range = env("EHL_ADDR_RANGE", "1-32").trim()
val timeoutMs = envLong("EHL_TIMEOUT_MS", 250)
val delayMs = envLong("EHL_DELAY_MS", 20)
...
println("Scanning addresses ${addrRange.first}..${addrRange.last} on ${cfg.portName} @ ${cfg.baudRate} (${env("EHL_PARITY", "NONE")}, stop=${env("EHL_STOP_BITS", "1")})")
...
runBlocking {
    for (addr in addrRange) {
        ...
        val ok = try {
            ...
            if (stateByte != null) {
                val s = interpretStateByte(stateByte)
                println("ADDR $addr: STATE raw=0x${"%02X".format(s.raw)} bits=${s.bits} open_for_delivery=${s.openForDelivery}")
            } else {
