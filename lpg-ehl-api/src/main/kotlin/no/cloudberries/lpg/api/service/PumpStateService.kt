...
class PumpStateService(
    ...
    fun initializePriceFromDatabase() {
        ...
        try {
            ...
            if (dbPrice != null) {
                ...
                // Broadcast til UI så Control Panel oppdateres umiddelbart
                logWebSocketHandler.broadcastPriceUpdate(priceKr)
    ...
    fun settle(address: Int = 1, paymentMethod: String = "CARD"): SettledTransaction? {
        ...
        if (transactionId != null) {
            ...
            try {
                val transaction = no.cloudberries.lpg.api.model.Transaction(
    ...
    private fun broadcastStatus(state: PumpState) {
        val statusMap = mapOf(
            ...
            "volumeLitres" to state.volumeLitres,
            "amountKr" to state.amountKr,
            "pricePerLitreKr" to state.pricePerLitreKr,
            "nozzleLifted" to state.nozzleLifted,
            "hasPendingTransaction" to state.hasPendingTransaction
        )
        logWebSocketHandler.broadcastPumpUpdate(statusMap)
    }
