...
fun printPacket(p: EhlPacket) {
    println("RX ${p.command.name} addr=${p.address} data=${fmtBytes(p.data)}")
}
...
runBlocking {
    ...
    runCatching {
        ...
        if (sb != null) {
            ...
            println("STATE: raw=0x${"%02X".format(s.raw)} bits=${s.bits} open_for_delivery=${s.openForDelivery} start=${s.startButtonPressed} automode=${s.automode}")
        } else {
            printPacket(resp)
        }
