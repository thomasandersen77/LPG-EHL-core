...
without mixing that work into the EHL RS-485 scripts in `python-test/`.
...
Edit `python-test/payment-terminal/config.json` with the terminal connection details.
...
```bash
cd python-test/payment-terminal
...
Uses the repo’s Linux-only raw serial helper (`python-test/serial_linux.py`).
...
By default, scripts write logs to `python-test/payment-terminal/logs/` (and also to stdout).
